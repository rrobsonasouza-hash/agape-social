...
export default function CestasPage() {
  ...
  const [entrada, setEntrada] = useState({ tipo: "CESTA_PRONTA" as "CESTA_PRONTA" | "ITEM_AVULSO", origem: "DOACAO" as "DOACAO" | "COMPRA_PAROQUIA", doadorId: "", itemId: "", quantidade: 1, valorTotal: 0, data: hoje });
  ...
  async function registrarEntrada() {
    ...
    await api.criarMovimentacao({ ...entrada, campanhaId, operacao: "ENTRADA", doadorNome: doador?.nome ?? "", itemNome: item?.nome ?? "", unidade: item?.unidade ?? "", observacoes: "", familiaId: "", familiaNome: "" });
