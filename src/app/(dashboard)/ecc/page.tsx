...
import {
  ...
  MapPin, Plus, RefreshCw, Search, Users, BarChart3, BadgeCheck, HandCoins,
} from "lucide-react";
...
const vazio: EccPainel = {
  encontros: [], casais: [], participacoes: [], equipe: [], programacao: [], tarefas: [], visitas: [], comunicacoes: [], documentos: [], credenciamentos: [], arrecadacoes: [], voluntarios: [],
...
export default function EccPage() {
  ...
  return <main className="mx-auto w-full min-w-0 max-w-7xl space-y-6">
    ...
    <nav className="grid max-w-full grid-cols-2 gap-2 rounded-2xl border bg-white p-2 shadow-sm sm:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-9">{[
      ["secretaria", "Secretaria", Users], ["visitas", "Visitas", House], ["mapa", "Mapa e distâncias", MapPin], ["cronograma", "Cronograma", Clock3], ["tarefas", "Tarefas", ClipboardCheck], ["equipes", "Equipes", HeartHandshake], ["credenciamento", "Credenciamento", BadgeCheck], ["arrecadacao", "Arrecadação", HandCoins], ["gestao", "Gestão e relatórios", BarChart3],
    ...
    {!carregando && edicao && aba === "arrecadacao" && <ArrecadacaoEccSection encontroId={encontroId} arrecadacoes={dados.arrecadacoes.filter((item) => item.encontroId === encontroId)} onSaved={carregar} />}
