...
import {
  ...
  MapPin, Plus, RefreshCw, Search, Users, BarChart3, BadgeCheck, ShoppingCart, Palette, Copy, Settings2, LockOpen, QrCode,
} from "lucide-react";
...
const vazio: EccPainel = {
  ...
  podeGerenciarPosEncontro: false,
};
...
export default function EccPage() {
  ...
  return <main className="mx-auto w-full min-w-0 max-w-7xl space-y-6">
    ...
    ].filter(([valor]) => (valor !== "visitas" || dados.podeGerenciarVisitas) && (valor !== "pos_encontro" || dados.podeGerenciarPosEncontro)).map(([valor, nome, Icon]) => <button key={String(valor)} onClick={() => { setAba(valor as typeof aba); setFormulario(null); setCasalEmEdicao(""); }} className={`inline-flex min-w-0 items-center justify-center gap-2 rounded-xl px-3 py-3 text-center text-sm font-black leading-tight ${aba === valor ? "bg-blue-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}><Icon className="shrink-0" size={17} /><span className="min-w-0">{String(nome)}</span></button>)}</nav>
