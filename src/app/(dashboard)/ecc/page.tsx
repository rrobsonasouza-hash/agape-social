...
import {
  ...
  MapPin, Plus, RefreshCw, Search, Users, BarChart3, BadgeCheck, ShoppingCart, Palette, Copy, Settings2, LockOpen,
} from "lucide-react";
...
const equipeInicial: EccEquipeFormData = {
  ...
  status: "CONVIDADO", dataEscala: "", horaInicio: "", horaFim: "", observacoes: "",
};
...
export default function EccPage() {
  const { listar, criarEncontro, atualizarEncontro, reabrirEncontro, criarCasal, atualizarCasal, vincularCasal: vincularCasalApi, adicionarEquipe,
  ...
  const [aba, setAba] = useState<"secretaria" | "visitas" | "mapa" | "cronograma" | "tarefas" | "equipes" | "credenciamento" | "circulos" | "arrecadacao" | "gestao">("secretaria");
  ...
  const [encontroEmEdicao, setEncontroEmEdicao] = useState("");
  const [configuracaoDuplicada, setConfiguracaoDuplicada] = useState(false);
  ...
  useEffect(() => {
    ...
    setEquipe((atual) => ({ ...atual, encontroId, dataEscala: atual.dataEscala || dados.encontros.find((item) => item.id === encontroId)?.dataInicio || "" }));
    setProgramacao((atual) => ({ ...atual, encontroId }));
    setTarefa((atual) => ({ ...atual, encontroId }));
  }, [dados.encontros, encontroId]);
  ...
  function salvarEncontro(event: FormEvent) { event.preventDefault(); void executar("encontro", () => encontroEmEdicao ? atualizarEncontro(encontroEmEdicao, encontro) : criarEncontro(encontro), encontroEmEdicao ? "Configuração da edição atualizada." : "Edição criada com sucesso."); }
  function abrirNovaEdicao() {
    setEncontroEmEdicao(""); setConfiguracaoDuplicada(false); setEncontro(encontroInicial); setFormulario("encontro");
  }
  function configurarEdicao() {
    if (!edicao) return;
    setEncontroEmEdicao(edicao.id); setConfiguracaoDuplicada(false);
    setEncontro({ numero: edicao.numero, nome: edicao.nome, tema: edicao.tema, lema: edicao.lema, dataInicio: edicao.dataInicio, dataFim: edicao.dataFim, prazoInscricao: edicao.prazoInscricao, local: edicao.local, capacidadeCasais: edicao.capacidadeCasais, status: edicao.status, observacoes: edicao.observacoes });
    setFormulario("encontro");
  }
  function duplicarConfiguracao() {
    if (!edicao) return;
    const proximoNumero = Math.max(0, ...dados.encontros.map((item) => item.numero)) + 1;
    setEncontroEmEdicao(""); setConfiguracaoDuplicada(true);
    setEncontro({ ...encontroInicial, numero: proximoNumero, nome: edicao.nome, tema: edicao.tema, lema: edicao.lema, local: edicao.local, capacidadeCasais: edicao.capacidadeCasais, observacoes: edicao.observacoes });
    setFormulario("encontro");
  }
  function reabrirEdicaoAtual() {
    if (!edicao || !window.confirm("Reabrir esta edição para correções? O histórico já consolidado será preservado.")) return;
    void executar("reabertura", () => reabrirEncontro(edicao.id), "Edição reaberta para correções. O histórico foi preservado.");
  }
  ...
  return <main className="mx-auto w-full min-w-0 max-w-7xl space-y-6">
    ...
    <section className="rounded-2xl bg-gradient-to-r from-blue-900 to-blue-600 p-6 text-white shadow-lg">
      <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-black tracking-[.18em] text-blue-100">EDIÇÃO EM ACOMPANHAMENTO</p><h2 className="mt-2 text-2xl font-black">{edicao ? `${edicao.numero}º ECC · ${edicao.nome}` : "Cadastre a primeira edição"}</h2><p className="mt-2 text-sm text-blue-100">{edicao ? `${encontroStatus[edicao.status]} · ${edicao.tema || "Tema não informado"}` : "O painel será organizado por edição."}</p></div><div className="flex flex-wrap gap-2">{edicao && <button type="button" onClick={configurarEdicao} className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2.5 text-sm font-black text-white hover:bg-white/20"><Settings2 size={16} />Configurar</button>}{edicao && <button type="button" onClick={duplicarConfiguracao} className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2.5 text-sm font-black text-white hover:bg-white/20"><Copy size={16} />Duplicar base</button>}<button onClick={abrirNovaEdicao} className="rounded-xl bg-white px-4 py-2.5 text-sm font-black text-blue-700"><Plus size={16} className="mr-2 inline" />Nova edição</button></div></div>
    ...
    <nav className="grid max-w-full grid-cols-2 gap-2 rounded-2xl border bg-white p-2 shadow-sm sm:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-10">{[
      ["secretaria", "Secretaria", Users], ["visitas", "Visitas", House], ["mapa", "Mapa e distâncias", MapPin], ["cronograma", "Cronograma", Clock3], ["tarefas", "Tarefas", ClipboardCheck], ["equipes", "Equipes", HeartHandshake], ["credenciamento", "Credenciamento", BadgeCheck], ["circulos", "Círculos e cuidados", Palette], ["arrecadacao", "Compras e doações", ShoppingCart], ["gestao", "Gestão e relatórios", BarChart3],
    ...
    {formulario && <section className="rounded-2xl border border-blue-200 bg-blue-50/60 p-5 shadow-sm"><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-black text-slate-900">{formulario === "encontro" ? encontroEmEdicao ? "Configurações da edição" : configuracaoDuplicada ? "Nova edição a partir da anterior" : "Nova edição" : formulario === "casal" ? "Cadastro do casal" : formulario === "equipe" ? "Equipe de trabalho" : formulario === "programacao" ? "Atividade do cronograma" : formulario === "voluntario" ? "Cadastrar cônjuge como voluntário" : "Tarefa da edição"}</h2><button onClick={() => setFormulario(null)} className="text-sm font-bold text-slate-600">Fechar</button></div>
      ...
      {formulario === "encontro" && <>{configuracaoDuplicada && <div className="mb-4 rounded-xl border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-900"><strong>Configuração-base copiada.</strong><p className="mt-1">Casais, presenças, doações e movimentações financeiras não serão copiados. Revise principalmente as novas datas.</p></div>}{encontroEmEdicao && encontro.status === "ENCERRADO" && <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950"><div><strong>Edição encerrada e protegida</strong><p className="mt-1">Reabra somente para corrigir a configuração. O histórico consolidado continuará preservado.</p></div><button type="button" disabled={salvando === "reabertura"} onClick={reabrirEdicaoAtual} className="inline-flex items-center gap-2 rounded-xl bg-amber-700 px-4 py-2.5 font-black text-white"><LockOpen size={17} />Reabrir para correção</button></div>}<form onSubmit={salvarEncontro}><fieldset disabled={encontroEmEdicao !== "" && encontro.status === "ENCERRADO"} className="grid gap-4 disabled:opacity-60 md:grid-cols-2 lg:grid-cols-4"><label className={label}>Número<input required className={campo} type="number" min="1" value={encontro.numero} onChange={(e) => setEncontro({ ...encontro, numero: Number(e.target.value) })} /></label><label className={`${label} lg:col-span-2`}>Nome<input required className={campo} value={encontro.nome} onChange={(e) => setEncontro({ ...encontro, nome: e.target.value })} /></label><label className={label}>Situação<select className={campo} value={encontro.status} onChange={(e) => setEncontro({ ...encontro, status: e.target.value as EccEncontroFormData["status"] })}>{Object.entries(encontroStatus).filter(([valor]) => valor !== "ENCERRADO").map(([v, n]) => <option key={v} value={v}>{n}</option>)}{encontro.status === "ENCERRADO" && <option value="ENCERRADO">Encerrado</option>}</select></label><label className={`${label} md:col-span-2`}>Tema<input className={campo} value={encontro.tema} onChange={(e) => setEncontro({ ...encontro, tema: e.target.value })} /></label><label className={`${label} md:col-span-2`}>Lema<input className={campo} value={encontro.lema} onChange={(e) => setEncontro({ ...encontro, lema: e.target.value })} /></label><label className={label}>Início<input required className={campo} type="date" value={encontro.dataInicio} onChange={(e) => setEncontro({ ...encontro, dataInicio: e.target.value })} /></label><label className={label}>Término<input required className={campo} type="date" value={encontro.dataFim} onChange={(e) => setEncontro({ ...encontro, dataFim: e.target.value })} /></label><label className={label}>Prazo de inscrição<input className={campo} type="date" value={encontro.prazoInscricao} onChange={(e) => setEncontro({ ...encontro, prazoInscricao: e.target.value })} /></label><label className={label}>Limite de casais<input className={campo} type="number" min="0" value={encontro.capacidadeCasais} onChange={(e) => setEncontro({ ...encontro, capacidadeCasais: Number(e.target.value) })} /><span className="text-xs font-normal text-slate-500">Use zero quando ainda não houver limite.</span></label><label className={`${label} md:col-span-2 lg:col-span-3`}>Local<input className={campo} value={encontro.local} onChange={(e) => setEncontro({ ...encontro, local: e.target.value })} /></label><label className={`${label} md:col-span-2 lg:col-span-4`}>Observações<textarea className={`${campo} min-h-24`} value={encontro.observacoes} onChange={(e) => setEncontro({ ...encontro, observacoes: e.target.value })} /></label><button disabled={salvando === "encontro"} className={botao}>{encontroEmEdicao ? "Salvar configurações" : "Criar edição"}</button></fieldset></form></>}
      ...
      {formulario === "equipe" && <form onSubmit={salvarEquipe} className="grid gap-4 md:grid-cols-2 lg:grid-cols-4"><label className={`${label} lg:col-span-2`}>Voluntário do ECC<select required className={campo} value={equipe.voluntarioId} onChange={(e) => { const selecionado = dados.voluntarios.find((item) => item.id === e.target.value); setEquipe({ ...equipe, voluntarioId: e.target.value, funcao: selecionado?.funcaoEcc || equipe.funcao }); }}><option value="">Selecione</option>{dados.voluntarios.map((v) => <option key={v.id} value={v.id}>{v.nome} · {v.funcaoEcc}</option>)}</select></label><label className={`${label} lg:col-span-2`}>Equipe<select required className={campo} value={equipe.equipe} onChange={(e) => setEquipe({ ...equipe, equipe: e.target.value })}><option value="">Selecione a equipe</option>{equipesEcc.map(([nome, descricao]) => <option key={nome} value={nome}>{nome} — {descricao}</option>)}</select></label><label className={label}>Função<input required className={campo} value={equipe.funcao} onChange={(e) => setEquipe({ ...equipe, funcao: e.target.value })} /></label><label className={label}>Dia da escala<input type="date" className={campo} value={equipe.dataEscala} onChange={(e) => setEquipe({ ...equipe, dataEscala: e.target.value })} /></label><label className={label}>Início<input type="time" className={campo} value={equipe.horaInicio} onChange={(e) => setEquipe({ ...equipe, horaInicio: e.target.value })} /></label><label className={label}>Fim<input type="time" className={campo} value={equipe.horaFim} onChange={(e) => setEquipe({ ...equipe, horaFim: e.target.value })} /></label><label className={label}><span className="mt-4 flex gap-2"><input type="checkbox" checked={equipe.coordenador} onChange={(e) => setEquipe({ ...equipe, coordenador: e.target.checked })} />Coordena a equipe</span></label><button disabled={salvando === "equipe"} className={botao}>Adicionar à escala</button></form>}
    ...
    {carregando ? <div className="rounded-2xl bg-white p-12 text-center text-slate-500">Carregando operação do ECC...</div> : !edicao ? <div className="rounded-2xl border bg-white p-12 text-center text-slate-500">Cadastre uma edição para começar.</div> : <>
      ...
      {aba === "equipes" && <EquipesEccSection encontroId={encontroId} dataInicio={edicao.dataInicio} equipes={equipes} tarefas={tarefas} presencas={dados.equipePresencas} onSaved={carregar} />}
    </>}
  </main>;
}
