...
export async function GET(request: NextRequest) {
  try { return NextResponse.json(await listarRegistros(request, "areas_pastorais", PERFIS)); }
  catch (error) { return respostaErroOperacional(error); }
}
export async function POST(request: NextRequest) {
  try { return NextResponse.json(await criarRegistro(request, "areas_pastorais", PERFIS), { status: 201 }); }
  catch (error) { return respostaErroOperacional(error); }
}
