...
export async function GET(request: NextRequest) {
  try { const usuario = await exigirUsuarioAtivo(request); const { paroquiaId } = await resolverParoquia(usuario.paroquiaId); const caminho = new URL(request.url).searchParams.get("caminho"); if (!caminho || caminho.split("/")[0] !== paroquiaId) throw new Error("Documento não informado ou sem permissão."); const assinatura = await supabaseAdmin().storage.from("agape-documentos").createSignedUrl(caminho, 60); if (assinatura.error) throw assinatura.error; return NextResponse.json({ url: assinatura.data.signedUrl }); }
