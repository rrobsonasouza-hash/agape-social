...
const entrada=z.discriminatedUnion("acao",[
  z.object({acao:z.literal("CADASTRAR"),titularNome:z.string().trim().min(3),conjugeNome:z.string().trim().default(""),cpf:z.string().trim().refine(v=>!v||v.replace(/\D/g,"").length===11,"CPF inválido."),telefone:z.string().trim().default(""),email:z.union([z.email(),z.literal("")]).default(""),cep:z.string().trim().refine(v=>!v||v.replace(/\D/g,"").length===8,"CEP inválido."),logradouro:z.string().trim().default(""),numero:z.string().trim().default(""),complemento:z.string().trim().default(""),bairro:z.string().trim().default(""),cidade:z.string().trim().default(""),estado:z.string().trim().max(2).default("")}),
...
export async function GET(req:NextRequest){try{const{supabase,paroquiaId}=await contexto(req);const inicio=new Date();inicio.setDate(1);const[contas,dizimistas,pagamentos]=await Promise.all([
  ...
  supabase.from("secretaria_dizimistas").select("id,titular_nome,conjuge_nome,cpf,telefone,email,cep,logradouro,numero,complemento,bairro,cidade,estado,ativo").eq("paroquia_id",paroquiaId).order("titular_nome"),
