...
export function ProtectedArea({ children }: { children: React.ReactNode }) {
  const { usuario, carregando, erroAcesso } = useAuth(); const { buscarContexto } = useParoquia(false); const router = useRouter(); const pathname = usePathname();
  useEffect(() => { if (!carregando && !usuario && !erroAcesso) router.replace("/login"); }, [carregando, erroAcesso, router, usuario]);
  useEffect(() => { if (!carregando && usuario?.role === "admin_plataforma" && !pathname.startsWith("/paroquias") && !pathname.startsWith("/usuarios")) buscarContexto().then((paroquia) => { if (!paroquia) router.replace("/central"); }).catch(() => router.replace("/central")); }, [buscarContexto, carregando, pathname, router, usuario]);
