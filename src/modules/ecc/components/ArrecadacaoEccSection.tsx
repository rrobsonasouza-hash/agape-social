...
const itensPorCategoria: Record<EccArrecadacao["categoria"], string[]> = {
  ALIMENTO: ["Arroz", "Feijão", "Óleo", "Açúcar", "Café", "Leite", "Macarrão", "Molho de tomate", "Farinha", "Sal", "Carne", "Frango", "Legumes", "Frutas", "Pães", "Bolos", "Doces"],
  BEBIDA: ["Água", "Água com gás", "Refrigerante", "Suco", "Café pronto", "Leite"],
  VALOR: ["Compras da cozinha", "Compras de bebidas", "Materiais do encontro", "Despesas gerais do ECC"],
  OUTRO: ["Descartáveis", "Material de limpeza", "Gás", "Gelo", "Guardanapos", "Papel toalha", "Produtos de higiene", "Material de secretaria"],
};
  ...
  return <section className="space-y-5">
    <article className="rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-white p-5 shadow-sm">
      ...
      {necessidadeAberta && <form id="form-necessidade-ecc" onSubmit={salvarNecessidade} className="mt-5 grid gap-3 rounded-xl border border-blue-200 bg-white p-4 md:grid-cols-2 lg:grid-cols-4">
        <label className={label}>Categoria<select className={campo} value={formNecessidade.categoria} onChange={(e) => setFormNecessidade({ ...formNecessidade, categoria: e.target.value as EccNecessidadeFormData["categoria"], item: "" })}>{Object.entries(nomesCategoria).map(([valor, nome]) => <option key={valor} value={valor}>{nome}</option>)}</select></label>
        <label className={`${label} lg:col-span-2`}>Item<select required className={campo} value={formNecessidade.item} onChange={(e) => setFormNecessidade({ ...formNecessidade, item: e.target.value })}><option value="">Selecione um item cadastrado</option>{formNecessidade.item && !itensPorCategoria[formNecessidade.categoria].includes(formNecessidade.item) && <option value={formNecessidade.item}>{formNecessidade.item} · cadastro anterior</option>}{itensPorCategoria[formNecessidade.categoria].map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
    ...
    {aberto && <form id="form-arrecadacao-ecc" onSubmit={salvar} className="grid gap-4 rounded-2xl border border-blue-200 bg-blue-50/40 p-5 md:grid-cols-2 lg:grid-cols-4">
      ...
      <label className={label}>Categoria<select className={campo} value={formulario.categoria} onChange={(e) => setFormulario({ ...formulario, categoria: e.target.value as EccArrecadacaoFormData["categoria"], item: "" })}>{Object.entries(nomesCategoria).map(([valor, nome]) => <option key={valor} value={valor}>{nome}</option>)}</select></label>
      <label className={`${label} md:col-span-1 lg:col-span-2`}>{formulario.categoria === "VALOR" ? "Finalidade do valor" : "Item da lista de compras"}<select required className={campo} value={formulario.item} onChange={(e) => setFormulario({ ...formulario, item: e.target.value })}><option value="">Selecione um item cadastrado</option>{formulario.item && !itensPorCategoria[formulario.categoria].includes(formulario.item) && <option value={formulario.item}>{formulario.item} · cadastro anterior</option>}{itensPorCategoria[formulario.categoria].map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
